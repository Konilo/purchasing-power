...
# Define variables
CONTAINER_NAME="purchasing_power_enrichment_dev"
IMAGE_NAME="purchasing_power_enrichment:dev"
DOCKERFILE="enrichment/Dockerfile.dev"
...
docker run \
    ...
    --name ${CONTAINER_NAME} \
    --network=host \
    --volume "${PWD}"/enrichment:/app/enrichment \
    --volume "${PWD}/shared":/app/shared \
    --volume "${PWD}/.vscode":/app/.vscode \
    ${IMAGE_NAME} \
